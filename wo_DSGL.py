...
class StaticSpatialGraph(nn.Module):
    """w/o DSGL: 只用静态 A 做 DiffusionSpatialAggregator，不再学习动态记忆图。"""
    def __init__(self, d_model: int = 48, L: int = 2, dropout: float = 0.1):
        super().__init__()
        self.agg = DiffusionSpatialAggregator(d_model=d_model, L=L, dropout=dropout)

    def forward(self, H_temp: torch.Tensor, A_static: torch.Tensor):
        if A_static is None:
            raise RuntimeError("A_static is required.")
...
class STLearner_wo_DSGL(nn.Module):
    """时间多粒度 + 静态扩散图 + 原 SoftDTW 语义。"""
    def __init__(self,
                 N: int,
                 d_model: int = 48,
                 H_future: int = 12,
        ...
        self.spatial_net = StaticSpatialGraph(d_model=d_model, L=diff_L, dropout=dropout)
...
class DSGFL_Wrapper_wo_DSGL(nn.Module):
    def __init__(self,
        ...
        self.st_learner = STLearner_wo_DSGL(
            N=num_nodes,
            d_model=d_model,
            H_future=horizon,
            out_dim=1,
...
def run_pems08_wo_DSGL(root, device="cuda", t_in=12, t_out=12):
    name_upper = "PEMS08"
    ...
    print("\n=== Start FL Training: PEMS08 w/o DSGL ===")
    runner.run(
    ...
    print(f"[Final] PEMS08 w/o DSGL: MAE={mae:.4f} RMSE={rmse:.4f} MAPE={mape:.2f}%")
...
if __name__ == "__main__":
    ...
    run_pems08_wo_DSGL(root=args.root, device=args.device)
