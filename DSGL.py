...
# 改动摘要：
# 1) 空间特征学习去掉两个“掩码”：
#    - 移除节点偏置图 A_bias（node_E1/node_E2 产生的相似度加权）
#    - 移除 PSI 调制（psi_mod 对注意输出的逐元素门控）
# 2) 语义空间特征学习仅保留 SoftDTW 构图，移除其后的 Q/K/V Attention：
#    - H_sem 输出为全零；A_sem 来自 SoftDTW 距离经 softmax(-dist) + top-l 稀疏化并行归一化
# ===============================================================
